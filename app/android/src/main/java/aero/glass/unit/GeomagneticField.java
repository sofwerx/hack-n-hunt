 ...
 * <a href="http://www.ngdc.noaa.gov/geomag/WMM/DoDWMM.shtml">
 *     http://www.ngdc.noaa.gov/geomag/WMM/DoDWMM.shtml</a>.
...
public class GeomagneticField {
    ...
    public GeomagneticField(float gdLatitudeDeg,
                            ...
                            long timeMillis) {
        final int maxN = G_COEFF.length; // Maximum degree of the coefficients.

        // We don't handle the north and south poles correctly -- pretend that
        // we're not quite at them to avoid crashing.
        float latitudeDeg = Math.min(90.0f - 1e-5f,
                Math.max(-90.0f + 1e-5f, gdLatitudeDeg));
        computeGeocentricCoordinates(latitudeDeg,
                gdLongitudeDeg,
                altitudeMeters);
        ...
        // this is much faster than calling Math.sin and Math.com maxN+1 times.
        float[] sinMLon = new float[maxN];
        float[] cosMLon = new float[maxN];
        sinMLon[0] = 0.0f;
        cosMLon[0] = 1.0f;
        ...
        double latDiffRad = Math.toRadians(latitudeDeg) - mGcLatitudeRad;
    ...
    /**
     * Geocentric latitude.
     * I.e. angle between closest point on the equator and this point, at the center of the earth.
     *
     * @param gdLatitudeDeg
    ...
    private static class LegendreTable {
         ...
         */
        LegendreTable(int maxN, float thetaRad) {
